...
$hora=date("H:i:s");
	...
	$this->Cell(40,5,'DESPACHO DE GRANOS BASICOS POR CLIENTE',0,0,'C');
	$this->Ln(6);
	$this->Cell(80);	
...
$suma_total=0;
$transacciones=0;
$result=mysql_query("SELECT a.*, b.* FROM tab_salida as a, tab_cliente as b WHERE a.id_cliente=b.id_cliente and a.id_cliente='$seleccion1' and a.peso_bruto!=0 and (a.fecha_entrada>= '$fecha_inicio_buscar' and a.fecha_entrada <= '$fecha_fin_buscar') and a.tipo_peso=1 group by a.id_lote");
...
$reporte=mysql_query("SELECT a.*, b.*, c.* FROM tab_salida as a, tab_cliente as b, tab_lote as c WHERE a.id_cliente=b.id_cliente and a.id_cliente=c.id_cliente and a.id_cliente='$seleccion1' and (a.fecha_entrada>= '$fecha_inicio_buscar' and a.fecha_entrada<= '$fecha_fin_buscar') and c.id_subproducto='$id_subproducto1' and c.id_producto='$id_producto1' and a.id_lote='$id_lote2' and a.bandera='2' and a.tipo_peso=1 group by c.id_subproducto, c.id_producto, a.entrada, a.id_lote",$con);	
