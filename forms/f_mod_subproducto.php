...
$tabla="SELECT *  FROM tab_subproducto where id_subproducto='$_POST[cod_prod_modif]' and id_empresa='$id_empresa'";
  $select = mysql_query($tabla);
while($row = mysql_fetch_array( $select ))
{
	$cod_producto_base=$row['id_producto'];
	$cod=$row['id_subproducto'];
	$nom=$row['nom_subproducto'];
	$desc=$row['desc_subproducto'];
	
}	
$tabla2="SELECT *  FROM tab_producto where id_producto='$cod_producto_base' and id_empresa='$id_empresa'";
$select2 = mysql_query($tabla2);
while($row2 = mysql_fetch_array( $select2 ))
{
	$nom_producto_principal=$row2['nom_producto'];
}
...
function cancelar(){
		document.location.href='f_subproducto.php';	
	}
function salirr()
 {	 
...
$resultado = autorizar_mod($nombre_usuario,$con_usuario,$activo,$empresa,$modificar);	
	...
	$tabla="SELECT *  FROM tab_subproducto where id_subproducto='".$_SESSION['codigo']."' and id_empresa='$id_empresa'";
 $result = mysql_query($tabla);
$array = mysql_fetch_assoc($result);
{
	$cod1=$array['id_subproducto'];
}
...
?>
<body class="container" onLoad="document.formulario.nom_producto2.focus();"> 
...
//$bandera = $_POST['bandera'];
$cod1 = strtoupper($_POST["id_producto2"]);
...
while($row5 = mysql_fetch_array( $select5 ))
{
	$usuario_actualiza=$row5['id_usuario'];
	 ...
	 if($nombre1==""){
      ...
      if(isset($cod1)){
		$sql= ("UPDATE tab_subproducto SET nom_subproducto='$nombre1',desc_subproducto='$desc1', id_usuario_modifica='$usuario_actualiza', fecha_modifica='$fecha', hora_modifica='$hora' WHERE id_subproducto='$cod1' and id_empresa='$id_empresa'");
		mysql_query($sql,$con);
	  }
 			if(mysql_error())
		  { 
...
if($error == "3")
 {
	 echo '<div class="alert alert-danger">
 						  <a href="f_subproducto.php" class="alert-link">No tiene permiso para modificar registro!!! Haga click para continuar .....</a>
						  </div>';
	  }	  
    ...
    <div class="col-md-13">
        <div class="panel panel-primary">
           ...
           <div class="panel-body" >
           	<form role="form" name="formulario"  method="post" action="f_mod_subproducto.php">
          ...
          <div class="form-group">
            ...
            <div class="col-md-12">
          <div class="form-group">
          		<label for="nombre_producto">Nombre del Producto Principal</label>
            	<input type="text" class="form-control input-lg" value="<?PHP echo $nom_producto_principal;?>" readonly name="nom_produc_principal" id="nom_produc_principal" autocomplete="off" style="background:#FFF" >
            </div>
          </div>
           </div><!--- FIN ROW-----> 
