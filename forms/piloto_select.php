...
$id_empresa=$_SESSION['id_empresa_silo']; // = id_empresa a la que pertenece 
  ...
  {
	 $consulta=mysql_query("SELECT * FROM tab_transportista WHERE id_cliente = '".$id_cliente2."' and id_empresa='".$id_empresa."'",$con);
     echo " <select name='id_piloto2' id='id_piloto2' class='chosen' style='width:100%; border: 1px solid #ddd; height: 46px; border-color: #66afe9; outline: 0;  -webkit-box-shadow: inset 0 1px 1px rgba(0,0,0,.075), 0 0 8px rgba(102, 175, 233, .6);     box-shadow: inset 0 1px 1px rgba(0,0,0,.075), 0 0 8px rgba(102, 175, 233, .6); border-radius: 4px;' >";
     echo "<option value='0'> SELECCIONE TRANSPORTISTA</option>";
							  
		while($valor=mysql_fetch_array($consulta)){
			$codigo_municipio= $valor['id_transportista'];
			$nombre= $valor['nom_transportista']." ".$valor['ape_transportista']." ---> ".($valor["placa_vehiculo"]);
