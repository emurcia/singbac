...
?>
<?PHP
$tabla="SELECT *  FROM tab_almacenaje where id_almacenaje='$_POST[r_recepcion]' and id_empresa='$id_empresa'";
$select = mysql_query($tabla,$con);
while($row = mysql_fetch_array($select))
{
			$cod=$row['id_almacenaje'];
			$control=$row['entrada'];
...
function salirr()
 ...
 function datos(){
	 window.open('../reportes/Rp_reversion_recepcion.php?id='+document.formulario1.reporte.value,target="_blank",'directories=no,status=yes, titlebar=no, Width=800; Height:600, scrollbars=yes, resizeable=no, directories=no, toolbar=no menubar=no, top=50,left=200');
...
<?php	              
					...
					if ($result> 0){ //si la variable tiene al menos 1 fila entonces seguimos con el codigo
					    while ($fila = mysql_fetch_array($result)) 
						{
							...
							$cod_entrada=$fila['reversion_entrada'];
...
$tabla_extrae="SELECT *  FROM tab_almacenaje where id_almacenaje='$cod1' and id_empresa='$id_empresa'";
$select_extrae = mysql_query($tabla_extrae,$con);
while($row_extrae = mysql_fetch_array($select_extrae))
{
			$cod=$row_extrae['id_almacenaje'];
			...
			$neto_sin_humedad_guarda=$row_extrae['neto_sin_humedad'];
			...
			$tipo_reversion="1"; // Almacenamiento de granos
		  ...
		  { 
			$error="1";
				  ...
				  $sql_inv= ("UPDATE `tab_inventario` SET `movimiento_lote`='$movimiento_lote_guardar',  peso_sin_humedad='$peso_sin_humedad_guardar', peso_sin_humedad_maximo='$peso_sin_humedad_maximo_guarda' WHERE id_lote='$id_lote_guarda' and id_empresa='$id_empresa'");
				   ...
				   $sql_contador= ("UPDATE `tab_contador` SET `total`='$transaccion',`reversion_entrada`='$entrada' WHERE codigo='$codigo1' and id_empresa='$id_empresa'");
				   mysql_query($sql_contador,$con);
				   
				   mysql_query("DELETE FROM tab_almacenaje WHERE id_almacenaje = '$cod' AND id_empresa='$id_empresa';",$con);
					$error="2";
			  }
	}//fin bandera ok		
...
<form role="form" name="formulario1"  method="post" action="f_revertir_recepcion.php">
    ...
    <input type="hidden"  name="reporte" value="<?PHP echo $cod; ?>">            
  					...
  					<div class="row" >
        					...
        					<div class="panel panel-primary">
           						<div class="panel-heading"><strong>Reversión  de Almacenamiento de Granos Básico</strong>				</div> <!-- PANEL 1 --->
           						<div class="panel-body" >
      					            <div class="row"><!--- INICIO ROW----->
                      		<div class="col-md-3">
                              <div class="form-group">
                                    <label for="origen_origen">Código</label>
                                    <input type="text" class="form-control input-lg" id="id_almacenaje" style="background:#FFF" readonly  value="<?PHP echo $cod; ?>"name="id_almacenaje" autocomplete="off">
                              </div>
                            </div>
                            <div class="col-md-3">
                              <div class="form-group">
