...
//<img src="logo/logo.png" width="80" height="66" />
	...
	$this->Cell(40,5,'REPORTE DE SALIDAS POR CLIENTE CONTROL DE CALIDAD',0,0,'C');
	$this->Ln(6);
	$this->Cell(80);	
...
$suma_total=0;
$transacciones=0;
$result=mysql_query("SELECT a.*, b.* FROM tab_salida as a, tab_cliente as b WHERE a.id_cliente=b.id_cliente and a.id_cliente='$seleccion1' and a.peso_bruto!=0 and (a.fecha_entrada>= '$fecha_inicio_buscar' and a.fecha_entrada <= '$fecha_fin_buscar') group by a.id_lote");
...
// ENCABEZADO DE TABLA
	...
	$pdf->SetFont('Arial','B',10);
	$pdf->Cell(3);
    $pdf->Cell(183,5,utf8_decode('N°')."      ".utf8_decode('Control')."      ".utf8_decode('Placas')."      ".utf8_decode('Neto')."         ".utf8_decode('Fecha ')."           ".utf8_decode('Hora')."         ".utf8_decode('Fecha ')."       ".utf8_decode('Hora')."         ".utf8_decode('Peso')."          ".utf8_decode('Tempe-')."      ".utf8_decode('Humedad'),0,0,'C',0);
	$pdf->Ln(4);
	$pdf->Cell(12);	
	$pdf->Cell(1,5,utf8_decode('  ')."         ".utf8_decode('       ')."      ".utf8_decode('    ')."      ".utf8_decode(' kg')."          ".utf8_decode('Entrada ')."        ".utf8_decode('Entrada')."     ".utf8_decode('Salida ')."      ".utf8_decode('Salida')."  ".utf8_decode('Volumétrico')."    ".utf8_decode('ratura')."      ".utf8_decode(''),0,0,'J',0);
	$pdf->Ln(7);	
	$pdf->Cell(1);
...
$reporte=mysql_query("SELECT a.*, b.*, c.* FROM tab_salida as a, tab_cliente as b, tab_lote as c WHERE a.id_cliente=b.id_cliente and a.id_cliente=c.id_cliente and a.id_cliente='$seleccion1' and (a.fecha_entrada>= '$fecha_inicio_buscar' and a.fecha_entrada<= '$fecha_fin_buscar') and c.id_subproducto='$id_subproducto1' and c.id_producto='$id_producto1' and a.id_lote='$id_lote2' and a.bandera='2' and a.peso_bruto!=0 group by c.id_subproducto, c.id_producto, a.entrada, a.id_lote",$con);	
	...
	$pdf->Cell(17);
    $pdf->Cell(5,5,utf8_decode($peso_neto1),0,0,'C');
	$pdf->Cell(6);
