...
$tabla_transportista=mysql_query("SELECT * FROM tab_transportista Where id_transportista='".$_SESSION['$id_transportista1']."' and id_empresa='$id_empresa';");
						 		while($valor5=mysql_fetch_array($tabla_transportista)){
									$nom_transportista1= $valor5["nom_transportista"]." ".$valor5["ape_transportista"]. " - ".$valor5["placa_vehiculo"];
								}	
if($id_cliente11==""){
...
if(empty($peso_unico1))
    $peso_unico1 = 0.00;
    
if(empty($peso_tara1))
    $peso_tara1 = 0.00;
    
if(empty($peso_bruto1))
    $peso_bruto1 = 0.00;
    
$sql= mysql_query("UPDATE `tab_contador` SET `total`='$transaccion',`servicio_bascula`='$entrada' WHERE codigo='$codigo1' and id_empresa='$id_empresa'", $con) or die(mysql_error());

$act=("UPDATE tab_transportista SET ocupado=1 WHERE id_transportista='$id_transportista1' and id_empresa='$id_empresa'");mysql_query($act,$con)or die(mysql_error());

$prod=("UPDATE tab_producto SET ocupado=1 WHERE id_producto='$id_producto1' and id_empresa='$id_empresa'"); 
mysql_query($prod,$con)or die(mysql_error());

$suprod=("UPDATE tab_subproducto SET ocupado=1 WHERE id_subproducto='$id_subproducto1' and id_empresa='$id_empresa'");mysql_query($suprod,$con)or die(mysql_error());

mysql_query("insert into tab_bascula(id_bascula, entrada, id_cliente, id_transportista, id_producto, id_subproducto, fecha_entrada, hora_entrada, fecha_salida, hora_salida, opcion_peso, peso_bruto, peso_tara, peso_unico, destino, observacion, id_empresa, id_usuario2, ocupado, fecha_usuario, hora_usuario, id_usuario_modifica, fecha_modifica, hora_modifica) values ('$cod_nue_ser', '$entrada', '$id_cliente1', '$id_transportista1', '$id_producto1', '$id_subproducto1', '$fecha_entrada1', '$hora_entrada1', '$fecha_salida1','$hora_salida1', '$opcion_peso1', '$peso_bruto1', '$peso_tara1', '$peso_unico1', '$destino1', '$observacion1', '$id_empresa', '$id_usuario',0, '$fecha_entrada', '$hora','$id_usuario','$fecha_entrada','$hora')",$con)or die(mysql_error()); 
...
<br><br><br><br>
 ...
 if($error == "1")
 {
	  echo '<div class="alert alert-success">
 			<a href="f_bascula.php" class="alert-link">Error en el procesamiento de datos!!! Haga click para continuar .....</a><br>'.$mensajeerror.'
						  </div>';
