      ...
      <div class="panel-heading"><strong>DATOS DEL CLIENTE "<?PHP echo $nom_cliente; ?>"</strong></div> 
       <div class="panel-body" >
          ...
          <table>
			<tbody>
            <?php
			$sql_silo="SELECT COUNT(a.id_silo) as numero_silos, b.id_silo FROM tab_silo as a, tab_lote as b WHERE b.id_silo=a.id_silo AND b.id_cliente='$cod_cliente'";
			...
			<thead><tr>
            		<th colspan="5"></th>
                	<th colspan="2">ALMACENAJE</th>
   					<th colspan="2">DESPACHO</th>
   					<th colspan="2">DISPONIBILIDAD</th> 
					<th rowspan="2">DISTRIBUCION (%)</th>                                       
				</tr>
                <tr>
                	<th>NUM</th>
					<th>LOTE</th>
			...
			for ($j=0;$j<=$c-1;$j++)
			{
				...
				$a++;
				$lote=$codigo[$j];
				$sql_entrada="SELECT sum(peso_bruto) as peso_brutoe, sum(peso_tara) as peso_tarae,round(sum(neto_sin_humedad),2) as sin_humedad_entrada FROM tab_almacenaje WHERE id_cliente='$cod_cliente' and id_lote='$lote' and bandera='2'";
				...
				$sql="SELECT sum(peso_bruto) as peso_brutos, sum(peso_tara) as peso_taras, round(sum(peso_sin_humedad),2) as sin_humedad FROM tab_salida WHERE id_cliente='$cod_cliente' and id_lote='$lote' and bandera='2'";
         ...
         <div class="col-md-1">
          ...
          <div class="col-md-10" align="center"> ALMACENAMIENTO DE GRANOS BASICOS
			...
			for ($j=0;$j<=$c-1;$j++)
			{
				...
				$a++;
				$lote=$codigo[$j];
				$sql_entrada="SELECT round(sum(peso_bruto),2) as peso_bruto, id_lote FROM tab_almacenaje WHERE id_cliente='$cod_cliente' and id_lote='$lote' and bandera='2'";
				$result_entrada = mysql_query($sql_entrada,$con);
				while($row_entrada = mysql_fetch_array($result_entrada))
					{
				...
				$sql_tara="SELECT round(sum(peso_tara),2) as peso_tara, id_lote FROM tab_almacenaje WHERE id_cliente='$cod_cliente' and bandera='2' and id_lote='$lote'";
				...
				$sql_humedad="SELECT round(sum(neto_sin_humedad),2) as neto_sin_humedad, id_lote FROM tab_almacenaje WHERE id_cliente='$cod_cliente' and bandera='2' and id_lote='$lote'";
         ...
         <div class="col-md-1">
          ...
          <div class="col-md-10" align="center"> DESPACHO DE GRANOS BASICOS
			...
			for ($j=0;$j<=$c-1;$j++)
			{
				...
				$a++;
				$lote=$codigo[$j];
				$sql_entrada="SELECT round(sum(peso_bruto),2) as peso_bruto FROM tab_salida WHERE id_cliente='$cod_cliente' and id_lote='$lote' and bandera='2'";
				$result_entrada = mysql_query($sql_entrada,$con);
				while($row_entrada = mysql_fetch_array($result_entrada))
					{
				...
				$sql_tara="SELECT round(sum(peso_tara),2) as peso_tara FROM tab_salida WHERE id_cliente='$cod_cliente' and bandera='2' and id_lote='$lote'";
				$result_tara = mysql_query($sql_tara,$con);
				while($row_tara = mysql_fetch_array($result_tara))
					{
				...
				$sql_humedad="SELECT round(sum(peso_sin_humedad),2) as neto_sin_humedad FROM tab_salida WHERE id_cliente='$cod_cliente' and bandera='2' and id_lote='$lote'";
				$result_humedad = mysql_query($sql_humedad,$con);
				while($row_humedad = mysql_fetch_array($result_humedad))
					{
    ...
    <!-- GRAFICA POR KARDEX -->
     <div class="row"><!--- INICIO----->  
         <div class="col-md-1">
          ...
          <div class="col-md-10" align="center"> KARDEX (KILOGRAMOS)
        <div class="contendor">
		<div id="consulta" style="overflow:scroll;height:auto;width:auto; overflow-y: hidden;" align="center">
			<table>
			<thead>
			...
			$c2=0;
			$num=1;
			$suma_kardex_brutoees=0;
			$suma_kardex_taraees=0;
			$suma_kardex_netoees=0;
			$suma_kardex_brutoses=0;
			$suma_kardex_tarases=0;
			$suma_kardex_netoses=0;
			
				$sql_kardexes="SELECT a.id_kardex, a.id_almacenaje as kardex_entra, a.id_salida as kardex_sal, a.fecha, a.hora, b.id_almacenaje as entrada, b.peso_bruto as pbruto_entra, b.peso_tara as ptara_entra, b.id_cliente as id_cliente_ent, b.id_transportista as id_trans_entra, c.peso_bruto as pbruto_sale, c.peso_tara as ptara_sale, c.id_cliente as id_cliente_sal, c.id_transportista as id_trans_sale, d.id_cliente FROM tab_kardex as a, tab_almacenaje as b, tab_salida as c, tab_cliente as d WHERE a.id_almacenaje=b.id_almacenaje and a.id_salida=c.id_salida and (c.id_cliente=d.id_cliente or b.id_cliente=d.id_cliente) and (b.bandera='2' or c.bandera='2') and d.id_cliente='$cod_cliente' order by a.fecha ASC, a.hora ASC ";
		$result_kardexes = mysql_query($sql_kardexes,$con);
		while($row_kardexes = mysql_fetch_array($result_kardexes))
		{
				if($row_kardexes['pbruto_entra']==0){
					$concepto="SALIDA";
					$row_kardexes['pbruto_entra']="";
					$row_kardexes['tara_entra']="";
					$id_trans=$row_kardexes['id_trans_sale'];
						$trans=mysql_query("SELECT * FROM tab_transportista WHERE id_transportista='$id_trans'",$con);
						$array2 = mysql_fetch_assoc($trans);
						{
						 $transportita=$array2['nom_transportista']." ".$array2['ape_transportita'];
				...
				if($row_kardexes['ptara_entra']==0){
					$row_kardexes['ptara_entra']="";
					}					
								
				if($row_kardexes['pbruto_sale']==0){
					$concepto="ENTRADA";
					$row_kardexes['pbruto_sale']="";
					$row_kardexes['ptara_sale']="";
					$id_trans=$row_kardexes['id_trans_entra'];
						$trans=mysql_query("SELECT * FROM tab_transportista WHERE id_transportista='$id_trans'",$con);
						$array2 = mysql_fetch_assoc($trans);
						{
						 $transportita=$array2['nom_transportista']." ".$array2['ape_transportita'];
				...
				if(($row_kardexes['pbruto_entra']-$row_kardexes['ptara_entra'])==0){
					$saldoes="";
					}else{
					$saldoes=($row_kardexes['pbruto_entra']-$row_kardexes['ptara_entra']);
					}	
					if($row_kardexes['pbruto_entra']==0){
					$concepto="SALIDA";
					$row_kardexes['pbruto_entra']="";
				...
				if(($row_kardexes['pbruto_sale']-$row_kardexes['ptara_sale'])==0){
					$saldo_salees="";
					}else{
					$saldo_salees=	($row_kardexes['pbruto_sale']-$row_kardexes['ptara_sale']);
					}	
				$suma_kardex_brutoees=$suma_kardex_brutoees+$row_kardexes['pbruto_entra'];
				$suma_kardex_taraees=$suma_kardex_taraees+$row_kardexes['ptara_entra'];
				$suma_kardex_netoees=$suma_kardex_netoees+$saldoes;
				$suma_kardex_brutoses=$suma_kardex_brutoses+$row_kardexes['pbruto_sale'];
				$suma_kardex_tarases=$suma_kardex_tarases+$row_kardexes['ptara_sale'];
				$suma_kardex_netoses=$suma_kardex_netoses+$saldo_salees;													
					
				echo "<tr><td>".$num;
				echo "</td><td>".parseDatePhp($row_kardexes['fecha']);
				echo "</td><td>".$concepto;
				echo "</td><td>".$transportita;						
				echo "</td><td>".$row_kardexes['pbruto_entra'];
				echo "</td><td>".$row_kardexes['ptara_entra'];
				echo "</td><td>".$saldoes	;
				echo "</td><td>".$row_kardexes['pbruto_sale'];
				echo "</td><td>".$row_kardexes['ptara_sale'];
				echo "</td><td>".$saldo_salees;
				echo "</td><td>".($saldo_kardexes=$saldo_kardexes+$saldoes-$saldo_salees);	
				echo "</td></tr>";
				$num++;
		}
				echo "<tr><td colspan='4'> TOTALES";
				echo "</td><td>".$suma_kardex_brutoees;
				echo "</td><td>".$suma_kardex_taraees;
				echo "</td><td>".$suma_kardex_netoees;	
				echo "</td><td>".$suma_kardex_brutoses;
				echo "</td><td>".$suma_kardex_tarases;
				echo "</td><td>".$suma_kardex_netoses;							
				echo "</td></tr>";
			?>
			</tbody>
			</table>
