...
<input type="hidden"  name="reporte" value="<?PHP echo $_SESSION['cod_despacho']; ?>"> 
 ...
 <?php
 			  if($cod_usuario_cliente==""){
			  	$salida="DESPACH-0000000".$id_empresa;
				...
				$Sqldespacho="SELECT a.* FROM tab_salida as a WHERE (a.tipo_peso=1 OR a.peso_completo=1) and a.nuevo_indicador=0 AND despacho_ajuste=0 AND a.bandera=2 AND a.id_empresa='$id_empresa' order by a.fecha_entrada desc, a.hora_entrada desc";
				
			  }else{
				// $fecha_inicio="2021/11/01";
				 $fecha_inicio="2021/10/06";
				$Sqldespacho="SELECT a.* FROM tab_salida as a, tab_detalle_cliente as d WHERE d.id_cliente_principal='$cod_usuario_cliente' AND a.id_cliente=d.id_cliente_secundario AND a.nuevo_indicador=0 AND a.despacho_ajuste=0 AND a.id_empresa='$id_empresa' AND a.despacho_ajuste='0' AND a.fecha_entrada>='$fecha_inicio' order by a.fecha_entrada desc, a.hora_entrada desc"; //
			  }
