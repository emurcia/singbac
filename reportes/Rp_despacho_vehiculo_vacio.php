...
// $id_despacho_buscar="DESPACH-003";
...
$result=mysql_query("SELECT * FROM tab_salida WHERE id_salida='$id_despacho_buscar'");
	...
	$id_silo=$row['id_silo'];
	//.number_format($disponible_almacen, 2, ".", ",");
...
//<img src="logo/logo.png" width="80" height="66" />
	...
	$pdf->Cell(40,5,'SALIDA DE PRODUCTOS - ENTRADA DE VEHICULOS',0,0,'C');
    ...
    $pdf->SetFont('Arial','',10);
	$pdf->Cell(20,5,$fecha_entrada,0,0,'J');
    $pdf->Ln(5);	
	$pdf->Cell(130);
	$pdf->SetFont('Arial','',10);
	...
	$pdf->Ln(0);
	$pdf->Cell(50);
	$pdf->Cell(0,5,utf8_decode('N° de Salida'),0,0,'C');
	$pdf->SetFont('Arial','',10);
	$pdf->Cell(-50);
	...
	$pdf->SetFont('Arial','',10);
	$pdf->Cell(22);
 	$pdf->MultiCell(80,5,utf8_encode($nom_cliente1));
	$pdf->Cell(17);
	$pdf->SetFont('Arial','',11);	
	$pdf->Cell(10,5,'Origen:',0,0,'J',0); 
	$pdf->SetFont('Arial','',10);
	$pdf->Cell(22);
 	$pdf->MultiCell(80,5,utf8_encode($nom_origen));
	$pdf->SetFont('Arial','',11);
	$pdf->Cell(17);
    $pdf->Cell(10,5,'Producto:',0,0,'J',0); 
	$pdf->SetFont('Arial','',10);
	$pdf->Cell(22);
 	$pdf->Cell(1,5,utf8_encode($nom_producto1),0,0,'J');
//	$pdf->Ln(5);
//	$pdf->SetFont('Arial','',11);
//	$pdf->Cell(17);
...
//	$pdf->SetFont('Arial','',10);
//	$pdf->Cell(22);
 //	$pdf->Cell(1,5,utf8_encode($nom_subproducto1),0,0,'J');
	$pdf->Ln(5);
	$pdf->SetFont('Arial','',11);
	$pdf->Cell(17);
	...
	$pdf->SetFont('Arial','',10);
	$pdf->Cell(22);
 	$pdf->MultiCell(140,5,utf8_encode($observacion1));
	$pdf->SetFont('Arial','',11);
	$pdf->Cell(17);
    $pdf->Cell(10,5,utf8_decode('Operador de Báscula:'),0,0,'J',0); 
	$pdf->SetFont('Arial','',10);
	$pdf->Cell(28);
	$pdf->MultiCell(80,5,utf8_decode($nombre_usuario));		
// 	$pdf->MultiCell(80,5,utf8_encode($_SESSION['nombre_usuario_silo']));
	//$pdf->Ln(5);	
	$pdf->SetFont('Arial','',12);
	$pdf->Cell(17);
	...
	$pdf->SetFont('Arial','',10);
	$pdf->Cell(22);
 	$pdf->cell(80,5,utf8_encode($nom_silo1));
	$pdf->Cell(-25);
    $pdf->Cell(10,5,utf8_decode('N° de Lote:'),0,0,'J',0); 
	$pdf->SetFont('Arial','',10);
	$pdf->Cell(22);
 	$pdf->cell(80,5,utf8_encode($num_lote1));
	$pdf->Ln(12);	
	...
	$pdf->Cell(50);
	$pdf->SetFont('Arial','',8);
	$pdf->Rect(55,117,30,0);
	$pdf->Cell(8,6,utf8_decode('Revisión Garita'),0,0,'J',0); 
	$pdf->SetY(117);
	$pdf->Cell(125);
	$pdf->Rect(133,117,30,0);
	$pdf->SetFont('Arial','',8);
	$pdf->Cell(8,6,utf8_decode('Operador de Planta'),0,0,'J',0);
	$pdf->SetFont('Arial','',8);
	$pdf->Rect(5,5,200,130);
	$pdf->SetY(125);
	$pdf->Ln(-1);
