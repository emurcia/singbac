...
$hora=date("H:i:s");
	...
	$this->Cell(40,5,'SALIDA DE PRODUCTOS - CONTROL DE CALIDAD',0,0,'C');
	$this->Ln(6);
	$this->Cell(100);	
...
$result=mysql_query("SELECT a.*, b.*, c.* FROM tab_salida as a, tab_cliente as b, tab_lote as c WHERE $where1",$con); //
}else{
$result=mysql_query("SELECT a.*, b.*, c.* FROM tab_salida as a, tab_cliente as b, tab_lote as c,  tab_detalle_cliente as decl, tab_indicadoresdespacho as rec WHERE $where",$con); //
  }
...
// ENCABEZADO DE TABLA
	...
	$pdf->Cell(-5);
    $pdf->Cell(250,5,utf8_decode('N°')."   ".utf8_decode('Control')."      ".utf8_decode('Placas')."         ".utf8_decode('Neto')."         ".utf8_decode('Fecha ')."         ".utf8_decode('Hora')."      ".utf8_decode('Hume')."   ".utf8_decode('Tempe')."   ".utf8_decode('Grano')."   ".utf8_decode('Grano')."  ".utf8_decode('Granos')."   ".utf8_decode('Impu')."  ".utf8_decode('Otras')."  ".utf8_decode('Retenc')."    ".utf8_decode('Calor')."  ".utf8_decode('Insecto')."   ".utf8_decode('Hongo')."  ".utf8_decode('Peso 100')."   ".utf8_decode('Long 20')."    ".utf8_decode('Den-')."     ".utf8_decode('Stress'),0,0,'J',0);
	$pdf->Ln(4);
	$pdf->Cell(6);	
	$pdf->Cell(1,5,utf8_decode('  ')."".utf8_decode('       ')."      ".utf8_decode('    ')."               ".utf8_decode(' kg')."          ".utf8_decode('entrada ')."     ".utf8_decode('entrada')."    ".utf8_decode('dad')."      ".utf8_decode('ratura')."     ".utf8_decode('bola')."      ".utf8_decode('chico')."   ".utf8_decode('rotos')."      ".utf8_decode('reza')."    ".utf8_decode('varie')."  ".utf8_decode('malla 1/4')."      ".utf8_decode('                               ')."   ".utf8_decode('')."   ".utf8_decode('granos')."    ".utf8_decode('granos')."      ".utf8_decode('sidad')."     ".utf8_decode('Crack'),0,0,'J',0);
	$pdf->Ln(7);	
	$pdf->Cell(-3);
...
$reporte=mysql_query("SELECT a.id_salida, a.id_transportista, a.id_variable, a.entrada, a.peso_bruto, a.peso_tara, a.hora_entrada, a.hora_salida, a.id_cliente, a.fecha_entrada, a.fecha_salida, a.peso_sin_humedad, b.*, c.*, d.* FROM tab_salida as a, tab_cliente as b, tab_lote as c, tab_indicadoresdespacho as d WHERE a.id_cliente=b.id_cliente and a.id_cliente=c.id_cliente and a.id_cliente='$id_cliente_buscar' and (a.fecha_entrada>= '$fecha_inicio_buscar' and a.fecha_entrada<= '$fecha_fin_buscar') and c.id_subproducto='$id_subproducto1' and c.id_producto='$id_producto1' and a.id_lote='$id_lote2' and  a.id_salida=d.id_salida AND a.bandera='2' and a.tipo_peso=2 and a.id_empresa='$id_empresa' group by c.id_subproducto, c.id_producto, a.entrada, a.id_lote ",$con);	
	...
	$pdf->Cell(7);
	$pdf->SetFont('Arial','',9);	
	$pdf->Cell(5,5,utf8_decode($calor1));	
	$pdf->Cell(7);
	$pdf->SetFont('Arial','',9);	
	$pdf->Cell(5,5,utf8_decode($insecto1));	
	$pdf->Cell(7);
	$pdf->SetFont('Arial','',9);	
	$pdf->Cell(5,5,utf8_decode($hongo1));	
	$pdf->Cell(8);
	$pdf->SetFont('Arial','',9);	
	$pdf->Cell(5,5,utf8_decode($peso_100_granos1));	
	$pdf->Cell(10);
	$pdf->SetFont('Arial','',9);	
	$pdf->Cell(5,5,utf8_decode($longitud_20_granos1));
	$pdf->Cell(8);
	$pdf->SetFont('Arial','',9);	
