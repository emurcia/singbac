...
if($cod_usuario_cliente!=''){
		...
		if($id_cliente2=="0" AND $id_lote2=="0"){
		$where = "a.id_cliente=b.id_cliente AND b.id_cliente=decl.id_cliente_secundario AND a.id_salida=rec.id_salida AND decl.id_cliente_principal='$cod_usuario_cliente' and a.id_lote=c.id_lote and (a.fecha_entrada>= '$fecha_inicio_buscar' and a.fecha_entrada <= '$fecha_fin_buscar') and a.peso_bruto!='0'  and a.bandera='2' AND a.id_empresa='$id_empresa' ";
		
		}else{
			if($id_cliente2!="0" AND $id_lote2=="0"){
			$where = "a.id_cliente=b.id_cliente AND b.id_cliente=decl.id_cliente_secundario AND a.id_salida=rec.id_salida and a.id_lote=c.id_lote and a.id_cliente='$id_cliente2' and (a.fecha_entrada>= '$fecha_inicio_buscar' and a.fecha_entrada <= '$fecha_fin_buscar') and a.peso_bruto!='0'  and a.bandera='2' AND a.id_empresa='$id_empresa' ";
			}else{
				$where = "a.id_cliente=b.id_cliente AND b.id_cliente=decl.id_cliente_secundario AND a.id_salida=rec.id_salida and a.id_lote=c.id_lote and a.id_cliente='$id_cliente2' and a.id_lote='$id_lote2' and (a.fecha_entrada>= '$fecha_inicio_buscar' and a.fecha_entrada <= '$fecha_fin_buscar') and a.peso_bruto!='0'  and a.bandera='2' AND a.id_empresa='$id_empresa'";
				  }
		}
}else{//inicia otra condicion
	if($id_cliente2=="0" AND $id_lote2=="0"){
$where1 = "a.id_salida=rec.id_salida AND a.id_cliente=b.id_cliente  and a.id_lote=c.id_lote and (a.fecha_entrada>= '$fecha_inicio_buscar' and a.fecha_entrada <= '$fecha_fin_buscar') and a.peso_bruto!='0'  and a.bandera='2' AND a.id_empresa='$id_empresa' ";
}else{
	if($id_cliente2!="0" AND $id_lote2=="0"){
	$where1 = "a.id_salida=rec.id_salida AND a.id_cliente=b.id_cliente and a.id_lote=c.id_lote and a.id_cliente='$id_cliente2' and (a.fecha_entrada>= '$fecha_inicio_buscar' and a.fecha_entrada <= '$fecha_fin_buscar') and a.peso_bruto!='0'  and a.bandera='2' AND a.id_empresa='$id_empresa' ";
	}else{
		$where1 = "a.id_salida=rec.id_salida AND a.id_cliente=b.id_cliente and a.id_lote=c.id_lote and a.id_cliente='$id_cliente2' and a.id_lote='$id_lote2' and (a.fecha_entrada>= '$fecha_inicio_buscar' and a.fecha_entrada <= '$fecha_fin_buscar') and a.peso_bruto!='0'  and a.bandera='2' AND a.id_empresa='$id_empresa'";
	      }
}
		} 

		
//$sql = "SELECT a.*, b.*, c.* FROM tab_almacenaje as a, tab_cliente as b, tab_lote as c WHERE $where";
if($cod_usuario_cliente==""){
//$cli="CLI-000".$id_empresa;
$sql=("SELECT a.*, b.*, c.*, rec.* FROM tab_salida as a, tab_cliente as b, tab_lote as c, tab_indicadoresdespacho as rec WHERE $where1"); //
}else{
$sql=("SELECT a.*, b.*, c.*, rec.* FROM tab_salida as a, tab_cliente as b, tab_lote as c,  tab_detalle_cliente as decl, tab_indicadoresdespacho as rec WHERE $where"); //
  }


$resultdos = mysql_query($sql,$con);
...
//		date_default_timezone_set('America/Mexico_City');
		...
		*/
		...
		$titulosColumnas = array('N°', 'CONROL', 'PLACA', 'PILOTO', 'PESO HUMEDO(KG)', 'PERDIDA DE PESO (KG)', 'PESO SECO (KG)', 'FECHA ENTRADA', 'HORA ENTRADA', 'HUMEDAD', 'TEMPERATURA', 'GRANO BOLA',  'GRANO CHICO', 'GRANOS ROTOS',  'IMPUREZA', 'OTRAS VARIEDADES', 'PIEDRAS', 'INFESTACIÓN', 'RETENCIÓN MALLA 1/4', 'GRANOS DAÑADOS', 'CALOR','INSECTO', 'HONGO', 'GERMINACIÓN', 'PESO DE 100 GRANOS', 'LONGITUD 20 GRANOS', 'DENSIDAD', 'AFLOTOXINAS', 'FUMONISINAS', 'VOMITOXINAS (DON)', 'STRESS CRACK', 'FLOTADORES');
		...
		$objPHPExcel->setActiveSheetIndex(0)
					...
					->setCellValue('U9',  $titulosColumnas[20])
					->setCellValue('V9',  $titulosColumnas[21])
					->setCellValue('W9',  $titulosColumnas[22])
					->setCellValue('X9',  $titulosColumnas[23])
					->setCellValue('Y9',  $titulosColumnas[24])
					->setCellValue('Z9',  $titulosColumnas[25])
					->setCellValue('AA9',  $titulosColumnas[26])
					->setCellValue('AB9',  $titulosColumnas[27])
					->setCellValue('AC9',  $titulosColumnas[28])
					->setCellValue('AD9',  $titulosColumnas[29])
					->setCellValue('AE9',  $titulosColumnas[30])
					->setCellValue('AF9',  $titulosColumnas[31]);
		...
		while($row_datos2 = mysql_fetch_assoc($result)) 
                            {	
							$numero++;
							$proveedor=$row_datos2[2];
							$fecha_entrada1=parseDatePhp($row_datos2['fecha_entrada']);
							...
							$infestacion1=$row_datos2['infestacion'];
							$retencion_malla1=$row_datos2['retencion_malla'];
							$suma_granos_danados1=$row_datos2['suma_granos_danados'];
							$calor1=$row_datos2['calor'];
							$insecto1=$row_datos2['insecto'];
							$hongo1=$row_datos2['hongo'];
							$germinacion1=$row_datos2['germinacion'];
							$peso_100_granos1=$row_datos2['peso_100_granos'];
							$longitud_20_granos1=$row_datos2['longitud_20_granos'];
							$densidad1=$row_datos2['densidad'];
							$aflotoxinas1=$row_datos2['aflotoxinas'];
							$fumonisinas1=$row_datos2['fumonisinas'];
							$vomitoxinas1=$row_datos2['vomitoxinas'];
							$stress_crack1=$row_datos2['stress_crack'];
							$flotadores1=$row_datos2['flotadores'];
		...
		$nomtranportista=$row_trans['nom_transportista'].' '.$row_trans['ape_transportista'];
					...
					->setCellValue('J'.$i, $humedad1)
					->setCellValue('K'.$i, $temperatura1)
					->setCellValue('L'.$i, $grano_bola1)
					->setCellValue('M'.$i, $grano_chico1)
					->setCellValue('N'.$i, $grano_roto1)
					->setCellValue('O'.$i, $impureza1)
					->setCellValue('P'.$i, $otras_variedades1)
					->setCellValue('Q'.$i, $piedras1)
					->setCellValue('R'.$i, $infestacion1)
					->setCellValue('S'.$i, $retencion_malla1)
					->setCellValue('T'.$i, $suma_granos_danados1)
					->setCellValue('U'.$i, $calor1)
					->setCellValue('V'.$i, $insecto1)
					->setCellValue('W'.$i, $hongo1)
					->setCellValue('X'.$i, $germinacion1)
					->setCellValue('Y'.$i, $peso_100_granos1)
					->setCellValue('Z'.$i, $longitud_20_granos1)
					->setCellValue('AA'.$i, $densidad1)
					->setCellValue('AB'.$i, $aflotoxinas1)
					->setCellValue('AC'.$i, $fumonisinas1)
					->setCellValue('AD'.$i, $vomitoxinas1)
					->setCellValue('AE'.$i, $stress_crack1)
					->setCellValue('AF'.$i, $flotadores1);
...
///--------------- IMPRIME EL CUADRO RESUMEN --------------------				
	...
	$estilofinColumnas = array(
		...
		*/
		...
		$objPHPExcel->getActiveSheet()->getStyle('A1:AF1')->applyFromArray($estiloTituloReporte);
		$objPHPExcel->getActiveSheet()->getStyle('A2:AF2')->applyFromArray($estiloTituloReporte);
