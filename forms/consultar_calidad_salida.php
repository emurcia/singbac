...
echo $cod_usuario_cliente=$fila_nom['id_cliente']; 
...
<script>
function excel(){
window.open('../reporteexcel/excel_calidad_salida.php?id='+document.formulario.id_cliente.value+'&id2='+document.formulario.fec_inicio.value+'&id3='+document.formulario.fec_fin.value+'&id4='+document.formulario.id_lote2.value,target="_blank",'directories=no,status=yes, titlebar=no, Width=800; Height:600, scrollbars=yes, resizeable=no, directories=no, toolbar=no menubar=no, top=50,left=200');	
}

function excel2(){
window.open('../reporteexcel/otro_excel_calidad_salida.php?id='+document.formulario.id_cliente.value+'&id2='+document.formulario.fec_inicio.value+'&id3='+document.formulario.fec_fin.value+'&id4='+document.formulario.id_lote2.value,target="_blank",'directories=no,status=yes, titlebar=no, Width=800; Height:600, scrollbars=yes, resizeable=no, directories=no, toolbar=no menubar=no, top=50,left=200');	
}

function reporte(){
window.open('../reportes/Rp_salida_calidad.php?id='+document.formulario.id_cliente.value+'&id2='+document.formulario.fec_inicio.value+'&id3='+document.formulario.fec_fin.value+'&id4='+document.formulario.id_lote2.value,target="_blank",'directories=no,status=yes, titlebar=no, Width=800; Height:600, scrollbars=yes, resizeable=no, directories=no, toolbar=no menubar=no, top=50,left=200');	

}

function reporte2(){
window.open('../reportes/Rp_salida_calidad2.php?id='+document.formulario.id_cliente.value+'&id2='+document.formulario.fec_inicio.value+'&id3='+document.formulario.fec_fin.value+'&id4='+document.formulario.id_lote2.value,target="_blank",'directories=no,status=yes, titlebar=no, Width=800; Height:600, scrollbars=yes, resizeable=no, directories=no, toolbar=no menubar=no, top=50,left=200');	
...
$sql=mysql_query("SELECT a.*, b.*, c.* FROM tab_salida as a, tab_cliente as b, tab_lote as c WHERE $where1",$con); //
}else{
	// b.id_cliente=decl.id_cliente_secundario where  and decl.id_empresa='$id_empresa'
	
	
	
$sql=mysql_query("SELECT a.*, b.*, c.* FROM tab_salida as a, tab_cliente as b, tab_lote as c,  tab_detalle_cliente as decl, tab_indicadoresdespacho as rec WHERE $where",$con); //
  }
...
<table border="0" align="center">
 ...
 <td width="20">&nbsp;</td>
 <td width="100"><button name="btn_excel" onclick="excel()" value="Exportar" class="btn btn-info btn-lg pull-right" > Exportar</button></td>
 <td width="20">&nbsp;</td>
 <td width="100"><button name="btn_excel1" onclick="excel2()" value="Exportar" class="btn btn-info btn-lg pull-right" > Exportar otros indicadores</button></td>
              	    </tr>
           	      </table>
