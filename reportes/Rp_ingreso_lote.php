...
$hora=date("H:i:s");
	...
	$this->Cell(40,5,'RECEPCION DE GRANOS BASICOS POR LOTE',0,0,'C');
	$this->Ln(6);
	$this->Cell(80);	
...
$suma_total=0;
$transacciones=0;
$result=mysql_query("SELECT a.*, b.*, c.* FROM tab_almacenaje as a, tab_cliente as b, tab_lote as c WHERE a.id_cliente=b.id_cliente and a.id_lote=c.id_lote and a.id_cliente='$seleccion1' and a.id_lote='$lote_busca' and (a.fecha_entrada>= '$fecha_inicio_buscar' and a.fecha_entrada <= '$fecha_fin_buscar') group by a.id_lote");
...
//	$nom_producto1=$row_producto['nom_producto'];
	 ...
	 if($row_datos['bandera']=="0"){ $lot_activo="ACTIVO";}
	 if($row_datos['bandera']=="1"){ $lot_activo="INACTIVO";}
...
$producto=mysql_query("SELECT * FROM tab_producto WHERE id_producto='$id_producto1'");
  while($row_producto = mysql_fetch_array($producto)){
	...
	$pdf->SetFont('Arial','B',10);
	$pdf->Cell(22);
 	$pdf->Cell(1,5,utf8_decode($lot_activo),0,0,'J');
	$pdf->Ln(5);		
	$pdf->Cell(17);
...
$reporte=mysql_query("SELECT a.*, b.*, c.* FROM tab_almacenaje as a, tab_cliente as b, tab_lote as c WHERE a.id_cliente=b.id_cliente and a.id_cliente=c.id_cliente and a.id_cliente='$seleccion1' and (a.fecha_entrada>= '$fecha_inicio_buscar' and a.fecha_entrada<= '$fecha_fin_buscar') and c.id_subproducto='$id_subproducto1' and c.id_producto='$id_producto1' and a.id_lote='$lote_busca' and a.bandera=2 and a.tipo_peso=2 group by a.entrada,c.id_subproducto, c.id_producto",$con);	
